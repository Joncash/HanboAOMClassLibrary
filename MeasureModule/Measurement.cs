...
namespace MeasureModule
{
	...
	public class Measurement
	{
		...
		public MeasureType ROIMeasureType;
		...
		private HTuple GenSurCircle2(HTuple circle, double width)
		{
			double row1 = circle[0];
			double col1 = circle[1];
			double radius = circle[2];
			...
			if (radius <= width)
				return new HTuple(new double[] { row1, col1, (radius + width), 0.0 });
			else
				return new HTuple(new double[] { row1, col1, (radius + width), (radius - width) });
		}
		...
		/// </summary>
		/// <returns></returns>
		public virtual MeasureViewModel GetViewModel()
		{
			return null;
		}
