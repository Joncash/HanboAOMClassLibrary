using HalconDotNet;
using Hanbo.CalibrationModule.Delegates;
using Hanbo.CalibrationModule.Models;
using System;
...
namespace Hanbo.CalibrationModule
{
	...
	{
		public event AddCalibImageCompletedDelegate On_AddCalibImageCompleted;
		...
		/// calibration image.
		/// </summary>
		public const string PS_NOT_FOUND = "找不到校正板 (Plate not found)";
		/// <summary>
		...
		/// </summary>
		public const string PS_MARKS_FAILED = "找不到標記 (Marks not found)";
		/// <summary>
		/// Constant indicating the result status of the 
		/// calibration preparation step:
		...
		/// </summary>
		public const string PS_QUALITY_ISSUES = "影像品質問題 (Quality issues detected)";
		/// <summary>
		/// Constant indicating the result status of the 
		/// calibration preparation step:
		...
		private BackgroundWorker _imgQualityExaminer;
		private List<string> _addImagTasks;// = new List<string>();
		private System.Windows.Forms.Timer _imageTaskTimer;// = new System.Timers.Timer(50);//interval = 50 ms
		#endregion
		...
		private void _imageTaskTimer_Tick(object sender, EventArgs e)
		{
			var timer = sender as System.Windows.Forms.Timer;
			timer.Stop();
			var doWork = (_addImagTasks.Count > 0) && !_imgQualityExaminer.IsBusy;
			if (doWork)
			{
				//doWork
				_imgQualityExaminer.RunWorkerAsync(_addImagTasks[0]);
			}
		}

		private void _imageTaskTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
		{
		...
		public void AddImageAsync(string filename)
		{
			_addImagTasks.Add(filename);
			_imageTaskTimer.Start();
		}

		#region 背景處理程序
		void _imgQualityExaminer_DoWork(object sender, DoWorkEventArgs e)
		{
			var worker = sender as BackgroundWorker;
			string imageFilepath = (string)e.Argument;
			if (File.Exists(imageFilepath))
			{
				var calibImage = addImage(imageFilepath);
				worker.ReportProgress(0, calibImage);
			}

		}
		void _imgQualityExaminer_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			var calibImage = e.UserState as CalibImage;
			if (On_AddCalibImageCompleted != null)
			{
				On_AddCalibImageCompleted(sender, calibImage);
			}
		}
		void _imgQualityExaminer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			if (_addImagTasks.Count > 0)
				_addImagTasks.RemoveAt(0);
			_imageTaskTimer.Start();
		}
		#endregion
		...
		public bool getCanCalibrate()
		{
			...
			for (int i = 0; i < count; i++)
				val += ((CalibImage)CalibData[i]).CanCalibFlag;
			...
			try
			{
				if (mSequenceTests < QUALITY_ISSUE_TEST_NONE)
				{
					...
					if (mSequenceTests == QUALITY_ISSUE_TEST_ALL && count > 0)
					{
						...
						width = new HTuple(((CalibImage)CalibData[0]).Width);
						height = new HTuple(((CalibImage)CalibData[0]).Height);
		...
		public HTuple getCalibratedPose(bool corrected)
		{
			...
			tX = new HTuple(0);
			tY = new HTuple(0);
			tZ = new HTuple(this.mThickness / -1000000.0);
		...
		public bool importCamParams(string camParFile)
		{
			...
			try
			{
				...
				mFocalLength = campar[0] * 1000000.0;
				...
				mCellWidth = campar[2 + offset] * 1000000000.0;
				mCellHeight = campar[3 + offset] * 1000000000.0;
