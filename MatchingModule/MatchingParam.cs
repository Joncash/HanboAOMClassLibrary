...
namespace MatchingModule
{
	...
	public class MatchingParam
	{
		...
		/// <summary>
		/// Matching Param 初始化預設值
		/// </summary>
		private void initializeDefaultValue()
		{
			this.mAngleExtent = 360.0 * Math.PI / 180.0;
			this.mAngleStep = (10.0 / 10.0) * Math.PI / 180.0;
			this.mContrast = 30;
			this.mMaxScale = 100.0 / 100.0;
			this.mMetric = "use_polarity";
			this.mMinContrast = 10;
			this.mMinScale = 100.0 / 100.0;
			this.mNumLevel = 5;
			this.mOptimization = "none";
			this.mScaleStep = 10.0 / 1000.0;
			this.mStartingAngle = 0.0 * Math.PI / 180.0;

			this.mMinScore = 50.0 / 100.0;
			this.mNumMatches = 1;
			this.mGreediness = 75.0 / 100.0;
			this.mMaxOverlap = 50.0 / 100.0;
			this.mSubpixel = "least_squares";
			this.mLastPyramidLevel = 1;

			this.mRecogRateOpt = 0;
			this.mRecogRate = 100;
			this.mRecogSpeedMode = MatchingParam.RECOGM_MANUALSELECT;
			this.mRecogManualSel = 1;
			this.mInspectMaxNoMatch = 1;
		}
		...
		private string[] diffAutoParams;
		public string[] GetDiffAutoParams()
		{
			return diffAutoParams;
		}
		...
		{
			var allAutoParamArray = new string[] { AUTO_ANGLE_STEP, AUTO_CONTRAST, AUTO_MIN_CONTRAST, AUTO_NUM_LEVEL, AUTO_OPTIMIZATION, AUTO_SCALE_STEP };
			allAutoParamArray = new string[] { AUTO_CONTRAST };
			var oriAutoParam = getAutoParList();
			diffAutoParams = oriAutoParam.Union(allAutoParamArray).Except(oriAutoParam.Intersect(allAutoParamArray)).ToArray();

			paramAuto.Clear();
			paramAuto.AddRange(allAutoParamArray);
		}
		...
		{
			var oriAutoParam = getAutoParList();
			var diffAutoParams = oriAutoParam;
			paramAuto.Clear();
		}
